...
def build_features():
    ...
    for src, typ in FPAIRS:
        ...
        for i in range(pf.num_row_groups):
            ...
            aa_start = ps if typ == "deletion" else pc.add(ps, 1)
...
SCOL = ["gene", "transcript", "indel_type", "aa_start", "indel_size_aa", "hgvs_p",
        ...
        "core_packing_loss", "ss_break", "hbond_loss", "hydrophobic_exposure", "n_mechanisms",
        "chrom", "pos", "ref", "alt"]
...
def score_block(prec, gen, typ):
    ...
    p = pq.read_table(pp, columns=["gene_symbol", "transcript", "protein_pos_start",
          ...
          "acmg_tier", "acmg_points", "mech_compact_core", "mech_ss_break",
    ...
    aa_start = ps if typ == "deletion" else ps + 1
    rel = pd.DataFrame({
        ...
        "acmg_tier": p.acmg_tier, "acmg_points": p.acmg_points.astype("int64"),
        "core_packing_loss": p.mech_compact_core.astype("int64"), "ss_break": p.mech_ss_break.astype("int64"),
